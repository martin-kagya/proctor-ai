...
from events import EventDetector, EventConfig, FLAG_LABELS
...
state = {"is_speaking": False, "yaw": 0, "pitch": 0, "roll": 0, "isCellphone_detected": False}
active_flags = {}
...
def main():
    ...
    try:
        ...
        while True:
            ...
            state["isCellphone_detected"] = obd.predict(frame)
            ...
            for ev in events:
                ...
                label = FLAG_LABELS.get(ev["type"], ev["type"].replace("_", " ").title())
                print(f"🚨 [Suspicious Event]: {label} ({ev})")
